...
def prototypical_loss(model_outputs, labels, n_support, n_query, dist='euclidean'):
    ...
    support_group_idxs = list(map(supp_idxs, classes)) #list: (opt.classes_per_it_tr, opt.num_support_tr), format: [tensor([ 67, 142, 257, 303, 420]), tensor([  7, 193, 307, 325, 350]), ····]
    prototypes = torch.stack([model_outputs[idx_list].mean(0) for idx_list in support_group_idxs]) # (batch, h' * w' * c')

    # 取support和query的样本，用来计算距离中心的距离
    support_idxs = torch.stack(list(map(lambda c: labels.eq(c).nonzero()[:], classes))).view(-1) # (n_classes * n_query)
    support_samples = model_outputs[support_idxs]
    ...
    support_dists = dist_loss(support_samples, prototypes, dist)
    query_dists = dist_loss(query_samples, prototypes, dist)

    log_p_y = F.log_softmax(-(support_dists + query_dists), dim=1).view(n_classes, n_query, -1) #(n_classes, n_query, n_prototypes(n_classes))
...
def prototypical_loss_cpu(model_outputs, labels, n_support, n_query, dist='euclidean'):
    '''
    Inspired by https://github.com/jakesnell/prototypical-networks/blob/master/protonets/models/few_shot.py
...
if __name__ == '__main__':
    ...
    labels = torch.arange(5).view(5, 1).expand(5, 20).reshape(100)
    loss = prototypical_loss_cpu(model_outputs, labels, 5, 15)
    print(loss)